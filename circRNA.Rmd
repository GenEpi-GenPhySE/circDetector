...
df_ssc_1 = df_ssc_1[df_ssc_1$readc > 3 & df_ssc_1$linear > 2, ]
df_ssc_2 = df_ssc_2[df_ssc_2$readc > 3 & df_ssc_2$linear > 2, ]
df_ssc_3 = df_ssc_3[df_ssc_3$readc > 3 & df_ssc_3$linear > 2, ]
...
ggp <- function(dataframe, title) {
  ...
  ggplot(dataframe) +
    ...
    labs(title=title, x = "mRNAseq expression level (read count/gene norm)",
                     y = "TotalRNAseq expression level (read count/gene norm)", fill="Gene") +
    theme(plot.title = element_text(hjust = 0.5), legend.position="bottom") +
    geom_point(data=dataframe,shape=21,aes(x=readc, y=linear, fill=col), size=1) +
    scale_fill_manual(values=c("dodgerblue2", "saddlebrown"), drop=FALSE) +
    geom_point(data = subset(dataframe, col == "- circRNA"), 
                aes(x = readc, y = linear, color = col), colour="dodgerblue2", size=1) +
    geom_smooth(method=lm, se=FALSE, data = subset(dataframe, col == "- circRNA"),
                                      aes(x = readc, y = linear, color = col), colour = "dodgerblue2") +
    stat_poly_eq(data = subset(dataframe, col == '- circRNA'), colour="dodgerblue2",
                 formula = "y~x", label.x = "left", label.y = 1,
                 aes(label = paste(..eq.label.., ..rr.label.., sep = "*`,`~"),
                 x = readc, y = linear, color = col), parse = TRUE)+
    geom_point(data = subset(dataframe, col == "+ circRNA"),
               aes(x = readc, y = linear, color = as.numeric(fr)), size=1)+ 
    geom_smooth(method=lm, se=FALSE, data = subset(dataframe, col == "+ circRNA"),
                                     aes(x = readc, y = linear, color = as.numeric(fr)),
                                     colour="saddlebrown")+
    stat_poly_eq(data = subset(dataframe, col == '+ circRNA'), colour="saddlebrown",
                 formula = "y~x",
                 aes(label = paste(..eq.label.., ..rr.label.., sep = "*`,`~"),
                 x = readc, y = linear, color = col), parse = TRUE) +
    scale_colour_gradient(low="saddlebrown", high="red", limits=c(7,13), name="log2(BSJ_reads_number)") 
    }
