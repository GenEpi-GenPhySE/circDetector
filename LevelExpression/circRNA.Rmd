...
ggplot(dataframe, aes(x=as.numeric(dataframe$linear), y=as.numeric(dataframe$readc), color = !is.na(dataframe$circ))) +
  ...
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE) +
    labs(title=title, x = "TotalRNAseq expression level (read count/gene norm)",
                      y = "mRNAseq expressino level (read count/gene norm)") +
  stat_fit_glance(method = "lm", label.x = "left", label.y = "top", size=3.5,
  ...
  scale_color_manual(values=c('dodgerblue2','saddlebrown'), name="Genes",
                     labels=c("Not involved in the production of circRNA",
                              "Involved in the production of circRNA")) +
  scale_fill_manual(values=c('dodgerblue2','saddlebrown')) +
  stat_ellipse(geom="polygon", aes(alpha = 0.4, fill = !is.na(dataframe$circ), show.legend = FALSE), show.legend = FALSE, type = "norm")
}
...
- Le décalage vers la droite des points marrons, montre que les gènes produisant des ARN circulaires sont « choisis » parmi ceux qui s’expriment par une quantité notable de transcrits polyA+ dans ce tissu

- Le décalage vers le bas des points marron montre que l’évaluation dans le total RNAseq conduit à une légère sur-évaluation (avec le testis-31, on devrait voir un bien plus gros décalage)
